...
NS_LOG_COMPONENT_DEFINE("TcpHighSpeednew");
NS_OBJECT_ENSURE_REGISTERED(TcpHighSpeednew);

TypeId
TcpHighSpeednew::GetTypeId()
{
    static TypeId tid = TypeId("ns3::TcpHighSpeednew")
                            .SetParent<TcpNewReno>()
                            .AddConstructor<TcpHighSpeednew>()
                            .SetGroupName("Internet");
    return tid;
}

TcpHighSpeednew::TcpHighSpeednew()
    : TcpNewReno(),
      m_ackCnt(0)
{
...
TcpHighSpeednew::TcpHighSpeednew(const TcpHighSpeednew& sock)
    : TcpNewReno(sock),
...
TcpHighSpeednew::Fork()
{
    return CopyObject<TcpHighSpeednew>(this);
}

/**
 * \brief Congestion avoidance of TcpHighSpeednew
 *
 ...
 *
 * As you can recall from RFC, Highspeednew works this way:
 *
 *               w = w + a(w)/w
 *
 ...
 * exactly the Highspeednew principle.
 *
 * \param tcb internal congestion state
 * \param segmentsAcked count of segments acked
 */
void
TcpHighSpeednew::CongestionAvoidance(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
    ...
    if (segmentsAcked > 0)
    {
        uint32_t coeffA = TableLookupA(segCwnd)/4;
...
std::string
TcpHighSpeednew::GetName() const
{
    return "TcpHighSpeednew";
}

/**
 * \brief Get slow start threshold following HighSpeednew principles
 *
 * \param tcb internal congestion state
 ...
 */
uint32_t
TcpHighSpeednew::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
    ...
    uint32_t ssThresh = static_cast<uint32_t>(std::max(segCwnd*7/8.0, segCwnd * b));
...
uint32_t
TcpHighSpeednew::TableLookupA(uint32_t w)
{
    if (w <= 38)
    {
        return 1;
...
double
TcpHighSpeednew::TableLookupB(uint32_t w)
{

...
struct edgestr{
    string parent, child;
    int weight;
    edgestr(string parent, string child, int weight){
        this->parent=parent;
        this->child=child;
        this->weight=weight;
    }
...
void input_weighted_edges(int&no_of_vertices, int&no_of_edges, vector<edgestr>&edges){
    for(int i=0;i<no_of_edges;i++){
        int weight;
        string pstr, cstr;
        cin>>pstr>>cstr>>weight;
        //cin>>parent>>child>>weight;
        edgestr newedge(pstr,cstr,weight);
        edges.push_back(newedge);
    }
}
...
void dijkstra(vector<vertex>& graph, int source) {
    ...
    while (!pq.empty()) {
        ...
        for (auto adj:graph[parent].list) {
            ...
            if (((graph[parent].distance + weight) < graph[child].distance)) {
...
void input_ports(int& n, vector<pair<string,int>>& port){
    cin>>n;
    while(n--){
        string s;
        int a;
        cin>>s>>a;
        port.push_back(make_pair(s,a));
    }
}
...
int main(){
    ...
    int n1, m1, n2, m2, c, d, n, m;
    vector<edge> edges;
    vector<edgestr> tempedges;
    vector<vertex> graph;
    cin>>n1>>m1>>n2>>m2;
    n=n1+n2;
    m=m1+m2;
    ...
    //input_weighted_edges(n,m,edges,states);
    input_weighted_edges(n1,m1,tempedges);
    input_weighted_edges(n2,m2,tempedges);
    cin>>c>>d;
    vector<pair<string,int>> port1, port2;
    int p1,p2;
    input_ports(p1,port1);
    input_ports(p2,port1);

    for(int i=0;i<p1;i++)
    for(int j=0;j<p2;j++){
        edgestr newedge(port1[i].first,port2[j].first,port1[i].second+port2[j].second);
        m++;
    }

    vector<string> mp;
    for(edgestr x:tempedges){
        bool found=false;
        for(auto y:mp)
            if(y==x.parent)
                found=true;
        if(!found)
            mp.push_back(x.parent);
    }
    for(edgestr x:tempedges){
        bool found=false;
        for(auto y:mp)
            if(y==x.child)
                found=true;
        if(!found)
            mp.push_back(x.child);
    }

    for(auto x:tempedges){
        int p,c,w;
        w=x.weight;

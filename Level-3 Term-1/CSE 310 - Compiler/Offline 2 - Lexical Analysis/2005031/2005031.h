...
class ScopeTable {
    ...
    public:
    // constructor
    ScopeTable (int totalBuckets, ScopeTable* parentScope, FILE *fp) {
...
class SymbolTable {
    ...
    SymbolTable (int totalBuckets, FILE* fp) {
        this->fp = fp;
        currentTable = new ScopeTable(totalBuckets, NULL, fp);
        this->totalBuckets = totalBuckets;
    }
    ...
    // member functions
    void enterScope () {
        ScopeTable* scopeTable = new ScopeTable(totalBuckets, currentTable, fp);
